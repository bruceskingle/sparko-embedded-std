...
pub struct SparkoEsp32StdInitializer {
    task_manager_builder: TaskSchedulerBuilder<SparkoEsp32Std>,
}
...
impl SparkoEmbeddedStdInitializer for SparkoEsp32StdInitializer {
    ...
    fn add_task(&mut self, task_initializer: Box<dyn ScheduledTask<SparkoEsp32Std>>, schedule_spec: &str) -> anyhow::Result<()> {
...
#[cfg(feature = "board-cyd")]
        let (remainder, spi, dc, back_light) = {
            ...
            let back_light: PinDriver<'static, esp_idf_hal::gpio::Output> = PinDriver::output(peripherals.pins.gpio21)?;
...
#[cfg(feature = "board-wave-esp32c6touch147")]
        let (remainder, spi, dc, back_light, reset, i2c, touch_driver_factory) = {
            ...
            let rotation: u16 = 0;
            let touch_driver_factory = TouchDriver::factory(
            ...
            (remainder, spi, dc, back_light, reset, i2c, touch_driver_factory)
        };
...
#[cfg(feature = "board-supermini-esp32c3")]
        let remainder = {
            use esp_idf_hal::{
                gpio::AnyIOPin, spi::{Dma, SpiConfig, SpiDeviceDriver, SpiDriver, SpiDriverConfig}, units::Hertz
            };
...
#[cfg(feature = "board-cyd")]
{
    ...
    use esp_idf_hal::{
        ...
        let di = crate::display_mipidsi::EspDi { spi: self.spi, dc: self.dc, xoffset:  0, yoffset: 0 };
        let mut delay = Ets;
...
#[cfg(feature = "board-wave-esp32c6147")]
{
    ...
    use esp_idf_hal::{
        ...
        let di = crate::display_mipidsi::EspDi { spi: self.spi, dc: self.dc, xoffset, yoffset };
        let mut delay = Ets;
...
#[cfg(feature = "board-wave-esp32c6touch147")]
{
    ...
    use crate::ahrs::qmi8658::Qmi8658;
        ...
        touch_driver = self.touch_driver_factory.build(172, 320, self.orientation)?;
...
impl SparkoEsp32Std {
    pub fn builder() -> anyhow::Result<(SparkoEsp32StdBuilder, Remainder)> {
